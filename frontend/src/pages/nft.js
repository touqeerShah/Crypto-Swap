...
export default function NFT() {
  ...
  const presaleMint = async () => {
    try {
      ...
      // update methods
      const nftContract = new Contract(ADDRESS.TKDevs, TK_NFT, signer);
      // call the presaleMint from the contract, only whitelisted addresses would be able to mint
      const tx = await nftContract.presaleMint({
        // value signifies the cost of one TK dev which is "0.01" eth.
      ...
      await tx.wait();
      setLoading(false);
      window.alert("You successfully minted a TK Dev!");
    } catch (err) {
      console.error(err);
    }
  };
  ...
  const publicMint = async () => {
    try {
      ...
      // update methods
      const nftContract = new Contract(ADDRESS.TKDevs, TK_NFT, signer);
      // call the mint from the contract to mint the TK Dev
